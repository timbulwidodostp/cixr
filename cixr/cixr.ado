...
The command also implements the "corrected" versions of the concentration index:
(i) the Erreygers correction (Correcting the Concentration Index, J Health Econ (2008),
doi:10.1016/j.jhealeco.2008.02.003), and 
(ii) the Wagstaff correction (The bounds of the concentration index when the variable of interest is binary, 
with an application to immunization inequality, Health Econ (2005, doi: 10.1002/hec.953)), which are appropriate 
for bounded health variables.

The Erreygers corrected concentration index rescales the CIX as:
    E(CIX) = 4 * mu * CIX 

The Wagstaff corrected concentration index rescales the CIX as:
    W(CIX) = CIX / (1 - mu)

where mu is the mean of the health variable.
...
*** CORRected option to present the "corrected" concentration index proposed by Erreygers and Wagstaff
...
09 October 2025 - fixed an issue where the graph would not generate when the number of observations was small and the quantiles parameter was set
05 May 2026 - inclusion of the Wasgtaff correction 
*********************************************************************
*/
...
quietly {
  ...
  *** MICRODATA ***
	...
	*** Calculate corrected concentration indices (Erreygers + Wagstaff)
  	if "`corrected'" ~= "" {
  	  ...
  	  noisily nlcom Erreygers_CI: ((8*var_rank)/(_max-_min))*_b[_rank], post noheader cformat(%9.4f)
  	  mat def ecix=e(b)
  	  mat def ecixse=e(V)
  	  return scalar ecix_se = sqrt(ecixse[1,1])
  	  return scalar ecix = ecix[1,1]
      return scalar ecix_p = 2*normal(-abs(ecix[1,1]/ecixse[1,1]))
 	
	  if "`cluster'" == "" reg `2' _rank [pw=_wt]
  	  else reg `2' _rank [pw=_wt], vce(cluster `cluster')

	  noisily display _newline `""Corrected" concentration index (Wagstaff, 2005):"'
      noisily nlcom Wagstaff_CI: ((2*var_rank)/(_b[_cons]+0.5*_b[_rank]))*_b[_rank] / (1 - (_b[_cons] + 0.5*_b[_rank])), post noheader cformat(%9.4f)

      mat def wcix = e(b)
      mat def wcixse = e(V)

      return scalar wcix_se = sqrt(wcixse[1,1])
      return scalar wcix = wcix[1,1]
      return scalar wcix_p = 2*normal(-abs(wcix[1,1]/wcixse[1,1]))
   }
